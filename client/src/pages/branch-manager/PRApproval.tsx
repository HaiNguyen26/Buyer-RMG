...
import {
  ...
  BRANCH_MANAGER_QUEUE_OPTIONS,
  approvalQueueEmptyHint,
  canBranchManagerActOnPr,
...
type BranchLineOutcome = 'APPROVED' | 'REJECTED' | 'ON_HOLD' | 'REVISION_REQUIRED';
...
function outcomeToModalAction(o: BranchLineOutcome): RejectLinesModalAction {
...
function modalActionToOutcome(a: RejectLinesModalAction): BranchLineOutcome {
...
function deptOutcomeLabel(o: string) {
...
function itemEligibleForBranchDecision(item: { status?: string; departmentItemOutcome?: string | null }) {
  if (!isNeedPurchaseItem(item)) return false;
...
const PRApproval = () => {
  ...
  const [showBranchDecisionModal, setShowBranchDecisionModal] = useState(false);
  const [branchDecisionKind, setBranchDecisionKind] = useState<'RETURN' | 'REJECT'>('RETURN');
  const [modalComment, setModalComment] = useState('');
  const [lineOutcomes, setLineOutcomes] = useState<Record<string, BranchLineOutcome>>({});
  ...
  const { data: prsData, isLoading, error } = useQuery({
    queryKey: ['branch-manager-pending-prs', queueFilter],
    queryFn: () => branchManagerService.getPendingPRs({ queue: queueFilter }),
    staleTime: 30000,
    gcTime: 5 * 60 * 1000,
    retry: 1,
  });
  ...
  const approvalPRs: any[] = useMemo(() => {
    let parsed: unknown = prsData;
    if (typeof prsData === 'string') {
      if (prsData.trim() === '') return [];
      try {
        parsed = JSON.parse(prsData);
      } catch {
        return [];
      }
    }
    ...
    if (Array.isArray(p.prs)) return p.prs as any[];
  ...
  const filteredPendingPRs = useMemo(() => {
    ...
    return approvalPRs.filter((pr) => {
      if (!q) return true;
      const blob = [pr.prNumber, pr.itemName, pr.requestor?.username, pr.department, pr.purpose]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
      return blob.includes(q);
    });
  }, [approvalPRs, searchQuery]);
  ...
  }, [isLoading, approvalPRs, location.pathname, location.state, navigate, searchParams, setSearchParams]);
  ...
  const approveMutation = useMutation({
    mutationFn: (payload: { prId: string; itemDecisions?: DepartmentItemDecisionPayload[] }) =>
      branchManagerService.approvePR(payload.prId, { itemDecisions: payload.itemDecisions }),
    onSuccess: () => {
      ...
      showSuccess('Đã gửi quyết định PR.');
    },
    onError: (err: any) => {
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['branch-manager-dashboard'] });
      setDetailPrId(null);
      setShowBranchDecisionModal(false);
      setModalComment('');
    },
  });
  ...
  const handleReturnClick = () => {
    setModalComment('');
    setBranchDecisionKind('RETURN');
    setShowBranchDecisionModal(true);
  };

  const handleRejectClick = () => {
    if (itemsRequiringBranchDecision.length > 0) {
      openRejectLinesModal();
      return;
    }
    setModalComment('');
    setBranchDecisionKind('REJECT');
    setShowBranchDecisionModal(true);
  };

  const handleBranchDecisionSubmit = () => {
    if (!detailPrId || !modalComment.trim()) return;
    if (branchDecisionKind === 'RETURN') {
  ...
  const decisionMutationPending = returnMutation.isPending || rejectMutation.isPending;

  const selectedPRData = approvalPRs.find((pr) => pr.id === detailPrId);

  const canActOnSelected = useMemo(
    () => canBranchManagerActOnPr(selectedPRData),
    [selectedPRData],
  );

  const itemsRequiringBranchDecision = useMemo(() => {
    if (!selectedPRData?.items) return [];
    return (selectedPRData.items as Array<any>).filter((it) => itemEligibleForBranchDecision(it));
  }, [selectedPRData]);

  const decidingItemIdSet = useMemo(
    () => new Set(itemsRequiringBranchDecision.map((x) => x.id)),
    [itemsRequiringBranchDecision],
  );

  useEffect(() => {
    if (!detailPrId || !selectedPRData?.items) return;
    const init: Record<string, BranchLineOutcome> = {};
    for (const it of itemsRequiringBranchDecision) {
      init[it.id] = 'APPROVED';
    }
    ...
    setShowRejectLinesModal(false);
    setRejectLinesDraft({});
  }, [detailPrId, selectedPRData?.id, itemsRequiringBranchDecision]);
  ...
  const handleApproveWithDecisions = () => {
    ...
    const anyApprovedNeed = itemsRequiringBranchDecision.some(
      (it) => (lineOutcomes[it.id] ?? 'APPROVED') === 'APPROVED',
    );
    if (itemsRequiringBranchDecision.length > 0 && !anyApprovedNeed) {
      showError(
        'Không còn dòng mua ngoài được duyệt — dùng «Xử lý dòng» hoặc «Từ chối PR» cho toàn bộ chứng từ.',
      );
      return;
    }
  ...
  const openRejectLinesModal = () => {
    ...
    for (const it of itemsRequiringBranchDecision) {
      const o = (lineOutcomes[it.id] ?? 'APPROVED') as BranchLineOutcome;
      draft[it.id] = { action: outcomeToModalAction(o), note: lineNotes[it.id] ?? '' };
    }
  ...
  const applyRejectLinesFromModal = () => {
    ...
    setShowRejectLinesModal(false);
    showSuccess('Đã cập nhật quyết định theo dòng. Nhấn «Duyệt & chuyển tiếp» để gửi.');
  };
      ...
      <div className={pageShellClass}>
        <div className={pageContentClass}>
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 ring-1 ring-rose-100">
            ...
            <button
              type="button"
              onClick={() => queryClient.invalidateQueries({ queryKey: ['branch-manager-pending-prs'] })}
        ...
        <BranchManagerPageHero
          ...
          rightSlot={
            <div className="rounded-xl border border-white/35 bg-white/15 px-4 py-2 text-center backdrop-blur-sm">
              ...
              <p className="text-2xl font-bold tabular-nums text-white">{filteredPendingPRs.length}</p>
        ...
        <div className={filterBarClass}>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              >
                {BRANCH_MANAGER_QUEUE_OPTIONS.map((opt) => (
        ...
        <div className="overflow-visible rounded-2xl shadow-[0_16px_30px_-20px_rgba(15,23,42,0.3)]">
          <div className={`${islandCard} overflow-hidden`}>
            <div className={departmentHeadListTableScrollClass}>
              <table
                className={`${departmentHeadInteractiveTableFixed880Class} ${saasTableRootClass} min-w-[1220px] whitespace-nowrap`}
              >
                <colgroup>
                  <col className="w-[18%]" />
                  <col className="w-[26%]" />
                        ...
                        <td className="px-4 py-2 align-middle whitespace-nowrap">
                          <div className={departmentHeadTableCellContentWrapClass}>
                            <div className="flex items-center gap-2">
                              <p className="block max-w-full truncate text-sm text-slate-700">
                                {pr.itemName || '—'}
                              </p>
                              {(pr.itemCount ?? 0) > 1 ? (
                ...
                <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 p-4 text-white shadow-md shadow-blue-500/20">
                  ...
                  <p className="mt-1 text-xl font-black tabular-nums leading-tight">
                    {selectedPRData.totalAmount
                      ? `${Number(selectedPRData.totalAmount).toLocaleString('vi-VN')} ${selectedPRData.currency || 'VND'}`
                      : '—'}
                  </p>
                  <DollarSign
              ...
              {selectedPRData.items && selectedPRData.items.length > 0 ? (
                <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                  ...
                  <p className="border-b border-slate-50 px-4 py-2 text-[11px] text-slate-500">
                    Dòng <strong className="text-slate-700">mua ngoài</strong> đã qua Trưởng phòng: chọn quyết định cấp chi
                    nhánh. Xử lý hàng loạt qua nút <strong className="text-slate-700">«Xử lý dòng»</strong>.
                  </p>
                    ...
                    <table className="w-full min-w-[720px] border-separate border-spacing-0 text-sm">
                      <thead className="sticky top-0 z-10 border-b border-slate-100 bg-slate-50">
                        <tr className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                          ...
                          <th className={`px-3 py-2.5 text-left ${saasTableHeadCellClass}`}>TP duyệt</th>
                          <th className={`px-3 py-2.5 text-left ${saasTableHeadCellClass}`}>GĐ CN quyết định</th>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {selectedPRData.items.map((item: any, idx: number) => {
                          ...
                          const serverDeptOutcome = String(item.departmentItemOutcome || '');
                          const branchOutcome = rowDeciding
                            ? (lineOutcomes[item.id] ?? 'APPROVED')
                            : need
                              ? serverDeptOutcome || 'APPROVED'
                              : 'APPROVED';
                          return (
                            <tr key={item.id || idx} className={idx % 2 === 1 ? 'bg-slate-50/40' : 'bg-white'}>
                              ...
                              <td className="px-3 py-2 align-middle">
                                {need ? (
                                  <span className="text-xs font-medium text-slate-600">
                                    {deptOutcomeLabel(serverDeptOutcome || 'APPROVED')}
                                  </span>
                                ) : (
                              ...
                              <td className="px-3 py-2 align-middle">
                                {rowDeciding ? (
                                  <select
                                    value={branchOutcome as BranchLineOutcome}
                                    onChange={(e) => {
                                      const v = e.target.value as BranchLineOutcome;
                              ...
                              <td className="px-3 py-2 align-middle">
                                {rowDeciding &&
                                (branchOutcome === 'REJECTED' || branchOutcome === 'REVISION_REQUIRED') ? (
                                  <input
                                    type="text"
...
{showRejectLinesModal && selectedPRData && typeof document !== 'undefined'
        ? createPortal(
            <div
              className="fixed inset-0 z-[10050] flex items-center justify-center bg-slate-900/55 p-4 backdrop-blur-[1px] isolation-isolate"
          ...
          >
            ...
            <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50/80 px-4 py-2.5 sm:px-5">
              {itemsRequiringBranchDecision.length > 0 ? (
                              ...
                              <fieldset className="min-w-0 space-y-1.5 border-0 p-0">
                                ...
                                {(
                                  ...
                                  >
                                    <input
                                      type="radio"
                                      name={`branch-line-outcome-${item.id}`}
      ...
      {showBranchDecisionModal && typeof document !== 'undefined'
        ? createPortal(
            <div
              className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm isolation-isolate"
          onClick={() => {
            setShowBranchDecisionModal(false);
            setModalComment('');
          }}
        >
          <div
            className="max-h-[90vh] w-full max-w-md animate-slideUpFadeIn overflow-hidden rounded-2xl bg-white shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div
              className={`flex items-center justify-between border-b border-slate-200 p-6 ${
                branchDecisionKind === 'REJECT'
            ...
            >
              <h3 className="flex items-center gap-2 text-lg font-bold text-slate-900">
                {branchDecisionKind === 'REJECT' ? (
                  <XCircle className="h-5 w-5 text-red-600" strokeWidth={2} aria-hidden />
                ) : (
                  <ArrowLeftRight className="h-5 w-5 text-amber-600" strokeWidth={2} aria-hidden />
                )}
                Phản hồi PR (cấp chi nhánh)
              </h3>
              <button
                type="button"
                onClick={() => {
            ...
            <div className="max-h-[calc(90vh-5rem)] space-y-4 overflow-y-auto p-6">
              <fieldset className="space-y-3 border-0 p-0">
                <legend className="text-sm font-medium text-slate-700">Chọn hình thức</legend>
                <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/80 p-3 hover:bg-slate-50">
                  <input
                    type="radio"
                    name="branch-pr-decision"
                    checked={branchDecisionKind === 'RETURN'}
                    onChange={() => setBranchDecisionKind('RETURN')}
                    className="mt-1 h-4 w-4 border-slate-300 text-amber-600 focus:ring-amber-500"
                  />
                  <span>
                    <span className="block text-sm font-semibold text-slate-900">Trả về chỉnh sửa</span>
                    <span className="mt-0.5 block text-xs leading-relaxed text-slate-600">
                      Requestor chỉnh PR và gửi lại; trạng thái{' '}
                      <strong className="font-medium">BRANCH_MANAGER_RETURNED</strong>.
                    </span>
                  </span>
                </label>
                <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/80 p-3 hover:bg-slate-50">
                  <input
                    type="radio"
                    name="branch-pr-decision"
                    checked={branchDecisionKind === 'REJECT'}
                    onChange={() => setBranchDecisionKind('REJECT')}
                    className="mt-1 h-4 w-4 border-slate-300 text-rose-600 focus:ring-rose-500"
                  />
                  <span>
                    <span className="block text-sm font-semibold text-slate-900">Từ chối hoàn toàn</span>
                    <span className="mt-0.5 block text-xs leading-relaxed text-slate-600">
                      Kết thúc duyệt tại cấp chi nhánh; trạng thái{' '}
                      <strong className="font-medium">BRANCH_MANAGER_REJECTED</strong>.
                    </span>
                  </span>
                </label>
              </fieldset>
              ...
              {branchDecisionKind === 'REJECT' ? (
                <div className="rounded-xl border border-red-200 bg-red-50 p-3">
                  <p className="text-sm font-medium text-red-900">
                    Từ chối hoàn toàn: PR không được chuyển tiếp procurement. Hãy chắc chắn đây không phải trường hợp chỉ
                    cần requestor chỉnh lại thông tin.
                  </p>
                </div>
              ) : null}
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700">
                  {branchDecisionKind === 'RETURN' ? 'Yêu cầu bổ sung / lý do trả' : 'Lý do từ chối'}{' '}
                ...
                <textarea
                  ...
                  placeholder={
                    branchDecisionKind === 'RETURN'
                      ? 'Nhập nội dung cần requestor bổ sung hoặc chỉnh sửa…'
                      : 'Nhập lý do từ chối PR…'
                  }
              ...
              <div className="flex items-center gap-3 pt-1">
                <button
                  ...
                  className="flex-1 rounded-xl bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-200"
                >
                  Hủy
                </button>
                <button
                  type="button"
                  onClick={handleBranchDecisionSubmit}
                  disabled={decisionMutationPending || !modalComment.trim()}
                  className={`flex-1 rounded-xl px-4 py-2 text-sm font-semibold text-white transition-colors disabled:opacity-50 ${
                    branchDecisionKind === 'REJECT'
