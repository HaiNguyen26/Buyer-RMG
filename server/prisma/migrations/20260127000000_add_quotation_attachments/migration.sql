-- Self-contained: ensure RFQ/quotation stack exists (also in 20260126500000; idempotent if that ran first).
-- Fixes P1014 shadow DB when `quotations` was never created by earlier migrations.
...
-- quotation_attachments (original migration; idempotent)
...
CREATE INDEX IF NOT EXISTS "quotation_attachments_company_id_idx" ON "quotation_attachments"("company_id");
CREATE INDEX IF NOT EXISTS "quotation_attachments_quotation_id_idx" ON "quotation_attachments"("quotation_id");
CREATE INDEX IF NOT EXISTS "quotation_attachments_deleted_at_idx" ON "quotation_attachments"("deleted_at");

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'quotation_attachments_quotation_id_fkey') THEN
    ALTER TABLE "quotation_attachments" ADD CONSTRAINT "quotation_attachments_quotation_id_fkey"
      FOREIGN KEY ("quotation_id") REFERENCES "quotations"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
  END IF;
END $$;
